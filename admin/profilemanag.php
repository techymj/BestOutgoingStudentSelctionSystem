...
$username=$_SESSION['Uname'];
    ...
    $getting_user_details_query = "SELECT * FROM `admin table` WHERE username='$username'";
    ...
    if(isset($_POST['upload_img'])){
        ...
        $file_allowed_type = array('jpg', 'jpeg', 'png','pdf');
        ...
        if(in_array($fileAcutalExt, $file_allowed_type)){
            if($fileUploadError === 0){
                if($fileSize < 20000000){
                    ...
                    $read_query = "UPDATE `admin table` set profile_image='$fileNewName' where id='$user_id'";
    ...
    if(isset($_POST['setting_submit'])){
        ...
        if(!empty($setting_username)){
            if ( !preg_match('/^[A-Za-z][A-Za-z0-9 ]{1,50}$/', $setting_username) ){
                ...
                $update_username_query = "update admin set username='$setting_username' where id='$user_id'";
       ...
       if(!empty($setting_email)){
            if(!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $setting_email)){
                ...
                $update_email_query = "update admin set email='$setting_email' where id='$user_id'";
        ...
        if(!empty($setting_password)){
            ...
            if($setting_password != $setting_re_password){
                ...
                if ( !preg_match("/^.*(?=.{8,56})(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).*$/", $setting_password) ){
                    ...
                    $update_password_query = "update admin set password='$setting_password' where id='$user_id'";
